        ...
        <div className="p-5 space-y-4 max-h-[75vh] overflow-y-auto">
          ...
          <div className="flex items-center justify-between text-xs bg-slate-950/50 p-2.5 rounded-lg border border-slate-800 text-slate-400">
            <div className="flex items-center gap-1.5">
              ...
              <span>{t("mapPopups.location") || "Location"}: {item.lat?.toFixed(4)}° N, {item.lng?.toFixed(4)}° E</span>
            </div>
            {item.state && (
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 text-center">
                  ...
                  <span className="text-[10px] text-slate-400 uppercase tracking-wider block">{t("mapPopups.rain24h") || "24h Rain"}</span>
                  <span className="text-sm font-bold text-cyan-300">{item.rainfall24h} mm</span>
                </div>

                <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 text-center">
                  <Droplets className="w-4 h-4 text-blue-400 mx-auto mb-1" />
                  <span className="text-[10px] text-slate-400 uppercase tracking-wider block">{t("mapPopups.soilSat") || "Soil Sat (VWC)"}</span>
                ...
                <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 text-center">
                  ...
                  <span className="text-[10px] text-slate-400 uppercase tracking-wider block">{t("mapPopups.slopeAngle") || "Slope Angle"}</span>
                ...
                <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 text-center">
                  ...
                  <span className="text-[10px] text-slate-400 uppercase tracking-wider block">{t("mapPopups.fsFactor") || "Fs (Safety)"}</span>
              ...
              <div className="bg-slate-950/70 p-3.5 rounded-xl border border-slate-800 space-y-2">
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-400">{t("mapPopups.porePressure") || "Pore Water Pressure"}:</span>
                  <span className="font-bold text-slate-200">{item.poreWaterPressure} kPa</span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-400">{t("mapPopups.displacement") || "Inclinometer Displacement"}:</span>
                  <span className="font-bold text-slate-200">{item.inclinometerDisplacement} mm/day</span>
                </div>
                <div className="flex justify-between items-center text-xs">
                  <span className="text-slate-400">{t("mapPopups.connectedHighway") || "Connected Highway"}:</span>
            ...
            <div className="space-y-3">
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2">
                <div className="flex justify-between items-center text-sm">
                  <span className="text-slate-400">{t("mapPopups.population") || "At Risk Population"}:</span>
                  <span className="font-bold text-white text-base">{item.population.toLocaleString()} Residents</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                  <span className="text-slate-400">{t("mapPopups.evacuationStatus") || "Evacuation Status"}:</span>
                  <span className="font-bold text-amber-400">{item.evacuationStatus}</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                  <span className="text-slate-400">{t("mapPopups.designatedShelter") || "Designated Shelter"}:</span>
              ...
              <div className="bg-red-950/40 border border-red-900/60 p-3 rounded-xl text-xs text-red-200">
                <span className="font-bold block mb-1">{t("mapPopups.primaryThreat") || "Primary Hazard Threat"}:</span>
                {item.threatReason}
              </div>

              <button
                onClick={() => dispatchNDRFUnit(item.id)}
                className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-red-600 to-amber-600 hover:from-red-500 hover:to-amber-500 text-white font-bold py-2.5 px-4 rounded-xl shadow-lg transition cursor-pointer"
              >
                <Truck className="w-4 h-4" />
                <span>{t("mapPopups.dispatchRescue") || "Dispatch NDRF/SDRF Rescue Unit"}</span>
              </button>
            </div>
          )}
            ...
            <div className="space-y-3">
              <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-slate-400">{t("mapPopups.highwayLength") || "Highway Length"}:</span>
                  <span className="font-bold text-slate-200">{item.lengthKm} km</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">{t("mapPopups.criticalCorridor") || "Critical Failure Corridor"}:</span>
                  <span className="font-bold text-amber-400">{item.criticalSegment}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">{t("mapPopups.trafficStatus") || "Traffic Status"}:</span>
              ...
              <div className="bg-amber-950/40 border border-amber-800/60 p-3.5 rounded-xl text-xs text-amber-200">
                <span className="font-bold block mb-1">{t("mapPopups.travelAdvisory") || "Travel Advisory"}:</span>
                {item.advisory}
              </div>
            </div>
          )}
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-950 p-3.5 rounded-xl border border-slate-800 space-y-1.5 text-xs">
                <div className="flex justify-between text-slate-400">
                  <span>{t("mapPopups.reportedBy") || "Reported By"}:</span>
                  <span className="font-bold text-slate-200">{item.author}</span>
                </div>
                <div className="flex justify-between text-slate-400">
                  <span>{t("mapPopups.hazardCategory") || "Hazard Category"}:</span>
